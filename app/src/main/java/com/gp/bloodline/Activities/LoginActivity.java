...
//Called when the activity is starting.
  ...
  private void login(final String number, final String password) {
    StringRequest stringRequest = new StringRequest(
      ...
      public void onResponse(String response) {
        if (!response.equals("رقم الجوال او الرقم السري غير صحيح")) {
      ...
      public void onErrorResponse(VolleyError error) {
        Toast.makeText(LoginActivity.this, "حدث شي خاطئ", Toast.LENGTH_SHORT).show();
  ...
  private boolean isValid(String number, String password) {
    if (number.isEmpty()) {
      showMessage("رقم الجوال فارغ");
      numberEt.setError("Empty Mobile Number");
      return false;
    } else if (password.isEmpty()) {
      showMessage("الرقم السري فارغ");
